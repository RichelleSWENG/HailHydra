...
public class ItemModel extends Model
{
	...
	public ResultSet getDetail(String ID)
	{
		...
		try
		{
			...
			String sql = "SELECT part_num, description, rack_location, stock_minimum, sister_company_price, traders_price, walk_in_price, last_cost, notes, image, status FROM item WHERE part_num = '"
					+ ID + "'";
	...
	public void addDetail(ArrayList list)
	{
		try
		{
			con = db.getConnection();
			statement = con.createStatement();
			String sql = "INSERT INTO item(part_num,description,rack_location,stock_minimum,sister_company_price,traders_price,walk_in_price,last_cost,notes,image,status) VALUES('"
					+ list.get(0)
					+ "','"
					+ list.get(1)
					+ "','"
	...
	public void editDetail(ArrayList list)
	{
		try
		{
			con = db.getConnection();
			statement = con.createStatement();
			String sql = "UPDATE item SET description='" + list.get(1)
					+ "',rack_location='" + list.get(2) + "',stock_minimum='"
					+ list.get(3) + "',sister_company_price='" + list.get(4)
					+ "',traders_price='" + list.get(5) + "',walk_in_price='"
					+ list.get(6) + "',last_cost='" + list.get(7) + "',notes='"
					+ list.get(8) + "',image='" + list.get(9) + "',status='"
					+ list.get(10) + "' WHERE part_num='" + list.get(0) + "'";
			statement.executeUpdate(sql);
		} catch (Exception e)
		{
			e.getMessage();
	...
	public ResultSet searchPrices(String field, String filter)
	{
		...
		{
			statement = con.createStatement();
			sql = "SELECT part_num,sister_company_price,sister_company_price,traders_price,traders_price,walk_in_price,walk_in_price FROM item WHERE "
					+ filter + " LIKE '%" + field + "%'";
			rs = statement.executeQuery(sql);
			rs.last(); // Get Item Count
			itemCount = rs.getRow();
	...
	public void changeWalkinPrice(String part_num, String wprice)
	{
		...
		{
			statement = con.createStatement();
			String sql = "UPDATE item SET walk_in_price='" + wprice
	...
	public void changeSisterCompanyPrice(String part_num, String sprice)
	{
		...
		{
			statement = con.createStatement();
			String sql = "UPDATE item SET sister_company_price='" + sprice
					+ "' WHERE part_num='" + part_num + "'";
			statement.executeUpdate(sql);
		} catch (Exception e)
		{
	...
	public void changeTradersPrice(String part_num, String tprice)
	{
		...
		{
			statement = con.createStatement();
			String sql = "UPDATE item SET traders_price='" + tprice
					+ "' WHERE part_num='" + part_num + "'";
			statement.executeUpdate(sql);
		} catch (Exception e)
		{
	...
	public ResultSet searchLastCost(String field, String filter)
	{
		...
		{
			statement = con.createStatement();
			sql = "SELECT part_num,description,last_cost,last_cost FROM item WHERE "
					+ filter + " LIKE '%" + field + "%'";
			rs = statement.executeQuery(sql);
			rs.last(); // Get Item Count
			itemCount = rs.getRow();
	...
	public void changeLastCost(String part_num, String last_cost)
	{
		...
		{
			statement = con.createStatement();
			String sql = "UPDATE item SET last_cost='" + last_cost
					+ "' WHERE part_num='" + part_num + "'";
			statement.executeUpdate(sql);
		} catch (Exception e)
		{
	...
	public ResultSet searchQuantity(String field, String filter)
	{
		...
		{
			statement = con.createStatement();
			sql = "SELECT part_num,description,quantity_functional,quantity_functional,quantity_defective,quantity_defective FROM item WHERE "
					+ filter + " LIKE '%" + field + "%'";
			rs = statement.executeQuery(sql);
			rs.last(); // Get Item Count
			itemCount = rs.getRow();
	...
	public void changeQuantity(String part_num, String functional,
			String defective)
	{
		try
		{
			statement = con.createStatement();
			String sql = "UPDATE item SET quantity_functional='" + functional
