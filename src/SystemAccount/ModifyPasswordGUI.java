...
public class ModifyPasswordGUI extends JPanel
{
	...
	private JLabel lblHeader, lblUserType, lblCurrentPassword, lblNewPassword,
			lblConfirmPassword, lblStatus;
	...
	public ModifyPasswordGUI(GUIController temp)
	{
		...
		pfConfirmPassword
				.setToolTipText("Enter the new password again for confirmation.");
		pfConfirmPassword.setBounds(220, 270, 395, 30);
		add(pfConfirmPassword);
		pfConfirmPassword.getDocument().addDocumentListener(
			...
			public void actionPerformed(ActionEvent e)
			{
				...
				if (mainController.checkOldPassword())
				{
					if (success == true)
					{
						...
						JOptionPane temp = new JOptionPane("" + type
				...
				} else
				{
					JOptionPane temp = new JOptionPane(
							...
							JOptionPane.INFORMATION_MESSAGE);
					JDialog dialog = temp
							.createDialog("Incorect Current Password");
					dialog.setAlwaysOnTop(true);
					dialog.setModal(true);
					dialog.setDefaultCloseOperation(JDialog.DISPOSE_ON_CLOSE);
					dialog.setVisible(true);
		...
		btnCancel.addActionListener(new ActionListener()
		{
			public void actionPerformed(ActionEvent e)
			{
				JOptionPane temp = new JOptionPane("Password not changed.",
						JOptionPane.INFORMATION_MESSAGE);
	...
	public boolean checkNewPassword()
	{
		...
		if (pfConfirmPassword.getPassword().length == pfNewPass.getPassword().length)
		{
			for (int i = 0; i < pfNewPass.getPassword().length; i++)
				if (pfNewPass.getPassword()[i] != pfConfirmPassword
						.getPassword()[i])
				{
					temp = false;
					break;
	...
	public void updateStatus()
	{
		...
		if (checkNewPassword())
		{
			if (pfNewPass.getPassword().length == 0
					&& pfConfirmPassword.getPassword().length == 0)
				lblStatus.setText("");
			else
			{
