    ...
    public ArrayList<Notification> getNotifs()
    {
        ...
        try
        {
            ...
            while (rs.next())
            {
                ...
                notifs.add(new Notification("Stock Minimum Reached", rs.getString("part_num") + ", quantity: " + rs.getInt("quantity_functional") + " has reached stock minimimum (" + rs.getInt("stock_minimum") + ")!"));
            }
            ...
            while (rs.next())
            {
                notifs.add(new Notification("Exceeded Terms", rs2.getString("name") + " (ID: " + rs.getString("company_id") + ") has exceeded its terms"));
            }
