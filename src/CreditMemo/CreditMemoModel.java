...
public class CreditMemoModel
{
			...
			String startDate, String endDate)
	{
		...
		try
		{
			...
			{
				sql = "SELECT creditmemo.date, creditmemo.credit_memo_id, company.name, creditmemo.return_slip_id,creditmemo.total_amount FROM company,returnslip,creditmemo WHERE returnslip.company_id=company.company_id AND creditmemo.return_slip_id=returnslip.return_slip_id AND creditmemo.date BETWEEN '"
						+ startDate
						+ "' AND '"
						+ endDate
			...
			} else if (filter.equalsIgnoreCase("credit memo number"))
			{
				sql = "SELECT creditmemo.date, creditmemo.credit_memo_id, company.name, creditmemo.return_slip_id,creditmemo.total_amount FROM company,returnslip,creditmemo WHERE returnslip.company_id=company.company_id AND creditmemo.return_slip_id=returnslip.return_slip_id AND creditmemo.date BETWEEN '"
						...
						+ "' AND '"
						+ endDate
						+ "' AND creditmemo.credit_memo_id LIKE '%"
						+ field
						+ "%'";
			} else if (filter.equalsIgnoreCase("return slip number"))
			{
				sql = "SELECT creditmemo.date, creditmemo.credit_memo_id, company.name, creditmemo.return_slip_id,creditmemo.total_amount FROM company,returnslip,creditmemo WHERE returnslip.company_id=company.company_id AND creditmemo.return_slip_id=returnslip.return_slip_id AND creditmemo.date BETWEEN '"
						...
						+ "' AND '"
						+ endDate
						+ "' AND creditmemo.return_slip_id LIKE '%"
						+ field
						+ "%'";
			}
			rs = statement.executeQuery(sql);
	...
	public void addCreditMemo(String cmID, String date, String rsNum,
			int status, String type, String partNum, String total)
	{
		...
		try
		{
			...
			statement = db.createStatement();
			String sql = "INSERT INTO creditmemo(credit_memo_id,date,return_slip_id,part_number,status,type,total_amount) VALUES('"
					+ cmID
					+ "','"
					+ date
					+ "','"
	...
	public CreditMemo getCM(String ID)
	{
		...
		{
			statement = db.createStatement();
			String sql = "SELECT * FROM creditmemo WHERE credit_memo_id = '"
					+ ID + "'";
			rs = statement.executeQuery(sql);
