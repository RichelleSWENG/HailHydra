...
public class ViewCreditMemoGUI extends CreditMemoGUI implements TableModelListener
{
               ...
               cmbSupplier.addActionListener(new ActionListener()
        {
            public void actionPerformed(ActionEvent e)
            {
                ...
                if (cmbSupplier.getSelectedIndex() != 0)
                {
                    ...
                    partNums = new String[RSController.getItems(c.getType()).size() + 1];
                    partNums[0] = "";
                    for (i = 1; i < RSController.getItems(c.getType()).size() + 1; i++)
                    {
                        partNums[i] = RSController.getItems(c.getType()).get(i - 1).getPartNum();
                    }
        
               TableColumn col = tbCreditMemo.getColumnModel().getColumn(1);
                    col.setCellEditor(new ViewCreditMemoGUI.MyComboBoxEditor(partNums));
                    col.setCellRenderer(new ViewCreditMemoGUI.MyComboBoxRenderer(partNums));
    ...
    public void tableChanged(TableModelEvent e)
    {
        if (e.getColumn() == 0)
        {
            ...
            if (tbModel.getValueAt(e.getFirstRow(), 1) != null)
            {
                ...
                if (Integer.valueOf(tbModel.getValueAt(e.getFirstRow(), 0).toString()) <= RSController.getAvailQuantity(Arrays.asList(partNums).indexOf(cmb)-1))
                {
                ...
                {
                    JOptionPane.showMessageDialog(null, "You can not buy that many items!!!! You can only buy " + RSController.getAvailQuantity(Arrays.asList(partNums).indexOf(cmb)-1) + ". Pls do not test me");
        ...
        if (e.getColumn() == 1)
        {
            if (tbModel.getValueAt(e.getFirstRow(), 1) != null)
            {
                ...
                if (!cmb.equals(""))
                {
                    ...
                    if (unique)
                    {
                        tbModel.setValueAt(RSController.getItems(c.getType()).get(Arrays.asList(partNums).indexOf(cmb)-1).getDescription(), e.getFirstRow(), 2);
                        tbModel.setValueAt(RSController.getItems(c.getType()).get(Arrays.asList(partNums).indexOf(cmb)-1).getPrice(), e.getFirstRow(), 3);
                    }
                    else
                    {
