...
public class BasicSystemSettingsGUI extends JPanel
{
	...
	private JLabel lblHeader, lblSystemName, lblVAT, lblVATPercentage,
			lblTermsAlert, lblTermsDays, lblCreditLimitAlert,
			lblCreditLimitPercentage, lblSystemStatus,
			lblAdministratorUsername, lblAdministratorPassword,
			lblAdministratorVerifyPassword, lblAdministratorStatus,
			lblEmployeeUsername, lblEmployeePassword,
	...
	private JPasswordField pfAdministratorPassword,
			pfAdministratorVerifyPassword, pfEmployeePassword,
	...
	public BasicSystemSettingsGUI(GUIController temp)
	{
		...
		add(pnlSystem);
		tbSystem = BorderFactory.createTitledBorder(
				BorderFactory.createLineBorder(Color.black), "System Account");
		...
		tbAdministrator = BorderFactory.createTitledBorder(
		...
		tfSystemName.getDocument().addDocumentListener(new DocumentListener()
		{
			...
			private void checkMaxNum()
			{
				if (tfSystemName.getText().length() > 60)
					lblSystemStatus
							.setText("System Name can not exceed 60 characters.");
		...
		ftfCreditLimitAlert.getDocument().addDocumentListener(
		...
		tfAdminUsername.getDocument().addDocumentListener(
				new DocumentListener()
				{
					public void changedUpdate(DocumentEvent documentEvent)
					{
					...
					private void checkMaxNum()
					{
						...
						if (tfAdminUsername.getText().length() > 45)
							lblAdministratorStatus
									.setText("Username can not exceed 45 characters.");
						else if (tfAdminUsername.getText().isEmpty()
								|| pfAdministratorPassword.getPassword().length == 0
								|| pfAdministratorVerifyPassword.getPassword().length == 0)
							lblEmployeeStatus
									.setText("Please fill-up everything.");
						else
		...
		pfAdministratorPassword.getDocument().addDocumentListener(
				new DocumentListener()
				{
					public void changedUpdate(DocumentEvent documentEvent)
					{
		...
		pfAdministratorVerifyPassword.getDocument().addDocumentListener(
				new DocumentListener()
				{
					public void changedUpdate(DocumentEvent documentEvent)
					{
		...
		tfEmployeeUsername.getDocument().addDocumentListener(
				new DocumentListener()
				{
					public void changedUpdate(DocumentEvent documentEvent)
					{
					...
					private void checkMaxNum()
					{
						...
						if (tfEmployeeUsername.getText().length() > 45)
							lblEmployeeStatus
									.setText("Username can not exceed 45 characters.");
						else if (tfEmployeeUsername.getText().isEmpty()
								|| pfEmployeePassword.getPassword().length == 0
								|| pfEmployeeVerifyPassword.getPassword().length == 0)
							lblEmployeeStatus
									.setText("Please fill-up everything.");
						else
		...
		pfEmployeePassword.getDocument().addDocumentListener(
				new DocumentListener()
				{
					public void changedUpdate(DocumentEvent documentEvent)
					{
		...
		pfEmployeeVerifyPassword.getDocument().addDocumentListener(
				new DocumentListener()
				{
					public void changedUpdate(DocumentEvent documentEvent)
					{
	...
	public boolean changeCreditLimitStatus()
	{
		...
		try
		{
			...
			{
				lblSystemStatus
						.setText("Credit limit percentage can not exceed 100%.");
				status = false;
			} else if (tfSystemName.getText().isEmpty())
				lblSystemStatus.setText("Please fill-up everything.");
			else
			...
			{
				lblSystemStatus
						.setText("Credit limit percentage can not be negative.");
				status = false;
			}
		} catch (Exception e)
		{
	...
	public void updateAdministratorStatus()
	{
		if (pfAdministratorPassword.getPassword().length <= 45)
		{
			...
			} else
			{
				lblAdministratorStatus.setForeground(Color.RED);
				lblAdministratorStatus
						.setText("New and confirm password do not match.");
			}
		} else
		{
			lblAdministratorStatus
					.setText("Password can not exceed 45 characters.");
		}
	}
