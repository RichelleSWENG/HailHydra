...
public class PaymentCollectiblesModel {
    ...
    public ResultSet viewPayment(String id,String type)
    {
        ...
        {
            statement = db.createStatement();
            String sql = "SELECT collection_id,date,amount,collection_type,debit_memo_id from collection where number='"+id+"' AND receipt_type='"+type+"'";
            rs = statement.executeQuery(sql);
        } catch (Exception e)
        {
            e.getMessage();
    ...
    public AcknowledgementReceipt getARDetails(String id)
    {
        ...
        {
            statement = db.createStatement();
            String sql = "SELECT name,original_amount,current_balance from acknowledgementreceipt,company where acknowledgement_receipt_id='"+id+"' AND acknowledgementreceipt.company_id=company.company_id";
            rs = statement.executeQuery(sql);
            while(rs.next())
            {
    ...
    public SalesInvoice getSIDetails(String id)
    {
        ...
        {
            statement = db.createStatement();
            String sql = "SELECT name,original_amount,current_balance from salesinvoice,company where sales_invoice_id='"+id+"' AND salesinvoice.company_id=company.company_id";
            rs = statement.executeQuery(sql);
            while(rs.next())
            {
